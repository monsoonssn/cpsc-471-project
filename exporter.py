...
def create_sequences(lines):
    ...
    for sequence in sequences:
        ...
        lines.append(
            'ALTER TABLE {0}"{1}" ALTER COLUMN "{2}" SET DEFAULT NEXTVAL(\'{0}"{1}_{2}_{3}"\');\n'.format(
                sequence["schema"],
                sequence["table"],
                sequence["column"],
                SEQUENCE_SUFFIX,
...
def set_schema(lines):
    ...
    if currSchema == None:
        print(
            "There is no schema set on the original script! Will default to public schema"
        )  # Not implemented: add schema to all statements
    else:
        for i in range(len(lines)):
            line = lines[i]
